...
class MLP(BaseModel):
    ...
    def __init__(
        ...
        for in_size, out_size in zip(self.layer_sizes[:-1], self.layer_sizes[1:]):
            self.layers.append({
                "weights": mul(randn([in_size, out_size]), 0.01),
                "bias": zeros([out_size]),
            })

    def summary(self):
        """Print model summary."""
        total_params = sum(to_numpy(layer["weights"]).size + to_numpy(layer["bias"]).size for layer in self.layers)
        print("MLP Summary:")
    ...
    def _output_activation_derivative(self, activated):
        return self._output_activation_derivative_fn(activated)

    def forward(self, X, training: bool = False):
        activations = [X]
        pre_activations = []
        dropout_masks: List[Optional] = []

        for index, layer in enumerate(self.layers):
            z = add(matmul(activations[-1], layer["weights"]), layer["bias"])
            ...
            if index < len(self.layers) - 1:
                ...
                if training and self.dropout > 0:
                    a_np = to_numpy(a)
                    mask = (np.random.rand(*a_np.shape) >= self.dropout).astype(np.float32) / (1 - self.dropout)
                    a = from_numpy(a_np * mask)
    ...
    def train(
        ...
        for epoch in range(self.epochs):
            ...
            for start in range(0, len(X), batch_size):
                ...
                delta = mul(error, self._output_activation_derivative(output))

                for layer_index in reversed(range(len(self.layers))):
                    a_prev = activations[layer_index]
                    w = self.layers[layer_index]["weights"]
                    a_prev_T = from_numpy(to_numpy(a_prev).T)
                    grad_w = div(matmul(a_prev_T, delta), len(batch_idx))
                    grad_b = mean(delta)

                    if self.l2 > 0:
                        grad_w = add(grad_w, mul(w, self.l2))

                    self.layers[layer_index]["weights"] = add(self.layers[layer_index]["weights"], mul(grad_w, self.learning_rate))
                    self.layers[layer_index]["bias"] = add(self.layers[layer_index]["bias"], mul(grad_b, self.learning_rate))

                    if layer_index > 0:
                        w_T = from_numpy(to_numpy(w).T)
                        delta = matmul(delta, w_T)
                        mask = dropout_masks[layer_index - 1]
                        if mask is not None:
                            delta = mul(delta, from_numpy(mask))
                        delta = mul(delta, self._activation_derivative(activations[layer_index]))
            ...
            if X_val is not None and y_val is not None:
                ...
                val_error = sub(y_val, val_output)
    ...
    def load(cls, path: str) -> "MLP":
        ...
        for index in range(int(data["n_layers"])):
            mlp.layers[index]["weights"] = from_numpy(data[f"weights_{index}"])
            mlp.layers[index]["bias"] = from_numpy(data[f"bias_{index}"])
