...
from .backend import randn, to_numpy, from_numpy, matmul, add, mul, sub, mean, pow
...
class Perceptron(BaseModel):
    ...
    def __init__(
        ...
        self.weights = mul(randn([input_size]), 0.01)
        self.bias = 0
    ...
    def forward(self, X):
        ...
        z = add(matmul(X, self.weights), self.bias)
    ...
    def train(
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, len(X_np), batch_size):
                ...
                # Backward pass
                d_error = mul(error, self._activate_derivative(output))
                
                # Update weights and bias
                X_batch_T = from_numpy(to_numpy(X_batch).T)
                weight_update = mul(matmul(X_batch_T, d_error), self.learning_rate / len(batch_indices))
                self.weights = add(self.weights, weight_update)
                
                bias_update = mul(mean(d_error), self.learning_rate)
                self.bias = add(self.bias, bias_update)
            
            # Average loss for epoch
            epoch_loss /= max(1, len(X_np) // batch_size)
    ...
    def load(cls, path: str) -> "Perceptron":
        ...
        perceptron.weights = from_numpy(data["weights"])
