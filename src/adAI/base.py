...
import logging
from .backend import to_numpy, sub, pow, mean, Backend, get_backend
...
class BaseModel(ABC):
    ...
    def _validate_training_data(
        ...
        if len(X_np) != len(y_np):
            raise ValueError(f"X and y must have same length, got {len(X_np)} and {len(y_np)}")
        ...
        if X_val is not None and y_val is not None:
            ...
            if len(X_val_np) != len(y_val_np):
                raise ValueError(f"X_val and y_val must have same length, got {len(X_val_np)} and {len(y_val_np)}")
    ...
    def _compute_metrics(
        ...
        y_true_flat = np.squeeze(y_true_np)
        y_pred_flat = np.squeeze(y_pred_np)
        
        diff = sub(y_true_flat, y_pred_flat)
        squared = pow(diff, 2)
        mse = mean(squared)
        mse_np = to_numpy(mse)
        
        diff_abs = np.abs(y_true_flat - y_pred_flat)
